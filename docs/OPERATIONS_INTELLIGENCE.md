...
The implementation follows this fixed pipeline:
...
The SHA-256 fingerprint uses stable, sorted meaningful snapshot content and
excludes `generatedAt` and the fingerprint field itself. It changes when
analytics evidence/scope changes, not because the response was generated at a
different instant.
...
evidence keys, severity/category/confidence changes, invalid drill-down scopes,
and numerical claims absent from referenced evidence. Displayed recommendation
...
automatic interpretation occurs only when the meaningful fingerprint or model
changes (or the process cache expires). Ask is separately rate-limited and
always begins with an approved read-only function call.
...
interpretation is unavailable and never fabricates a response.
