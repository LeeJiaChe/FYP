...
## 1. Core KPIs & Formulations

### 1.1. Boarded Passengers (Actual Ridership)
...
### 1.2. Reserved Seat-Segment Utilization
...
* **Exclusions**: Excludes cancelled Trips and future unperformed Trips to prevent artificial denominator dilution.
...
* **Exclusions**: Trips with no recorded origin departure timestamp.
...
### 1.4. Average & Maximum Departure Delay
...
* **Decision Utility**: Informs the penalty enforcement threshold and overbooking allowance parameters.
* **Limitations**: Sample confidence requires at least 10 eligible outcomes before triggering automated warnings.

---

### 1.6. Unserved Demand
* **Definition**: The number of student transit attempts that could not be accommodated due to capacity depletion.
* **Formula**:
  $$\text{Unserved Demand} = |\{w \mid w.\text{status} = \text{'EXPIRED'}\}| + |\{i \mid i.\text{status} = \text{'REJECTED\_FULL'}\}|$$
* **Exclusions**: Promoted waitlists (`PROMOTED`) and boarded walk-ins (`BOARDED`) are not unserved because capacity was fulfilled.
* **Decision Utility**: Quantifies unmet student demand per Service Line to justify additional bus frequency or fleet expansion.

---

### 1.7. Operational Cancellations & Administrative Exclusions
* **Definition**: Cancelled Trips that represent actual service reliability failures.
* **Exclusion Rule**: Trips cancelled with authoritative reasons matching administrative maintenance/rollover (e.g. `"Stale prototype..."`, `"Shared development migration..."`) are classified as `excludedAdministrativeCleanupTrips` and excluded from service quality penalties.
* **Decision Utility**: Protects historical service reliability metrics from distortion caused by system data cleanup activities.

---

### 1.8. Fleet Asset Utilization & Service Hours
...
## 2. Rule-Based Operational Insights & Thresholds
...
## 3. FYP Defense Questions & Answers
