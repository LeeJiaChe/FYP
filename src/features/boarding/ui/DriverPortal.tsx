...
export default function DriverPortal({
  ...
  async function manualReservedBoarding(passenger: ManifestPassenger) {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Manual boarding failed",
      );
    }
  }
  ...
  async function manualAlight(passenger: ManifestPassenger) {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Manual alighting failed",
      );
    }
  }
  ...
  const onBoard =
    manifest?.manifest.filter(
      (passenger) => passenger.boarded && !passenger.alighted,
    ) ?? [];
  ...
  function manifestRows(
    ...
    kind: "board" | "alight" | "onboard" | "all",
  ) {
    return (
      <div className="manifest-rows">
        {passengers.map((passenger) => (
          <article
            key={`${kind}-${passenger.kind}-${passenger.recordId}`}
          ...
          >
            ...
            <div className="manifest-state">
              ...
              <strong
                className={`manifest-state-label ${passenger.alighted ? "is-alighted" : passenger.boarded ? "is-onboard" : "is-waiting"}`}
              >
                {passenger.alighted
                  ? "Alighted"
                  : passenger.boarded
            ...
            {(kind === "board" && passenger.kind === "RESERVED") ||
            kind === "alight" ? (
              <div className="manifest-action">
                {kind === "board" && passenger.kind === "RESERVED" && (
                  <button
                    type="button"
                    onClick={() => void manualReservedBoarding(passenger)}
                ...
                {kind === "alight" && (
                  <button
                    type="button"
                    onClick={() => void manualAlight(passenger)}
                    className="btn-secondary"
                  >
                    Confirm alighted
                  </button>
  ...
  function manifestEmptyMessage(kind: "board" | "alight" | "onboard") {
    if (kind === "board")
      return manifest?.currentStop
        ? "No passengers waiting to board at this stop"
        : "No passengers currently waiting to board";
    if (kind === "alight")
      return manifest?.currentStop
        ? "No passengers alighting at this stop"
        : "No passengers currently due to alight";
    return "No passengers currently on board";
  }

  function primaryAction() {
    if (!manifest || terminal) return null;
    if (manifest.trip.status === "NOT_STARTED")
      return (
        <button
          type="button"
          onClick={() => void progress("START_BOARDING")}
          className="driver-primary-action"
        >
          <Play aria-hidden /> Start boarding
        </button>
      );
    if (manifest.trip.status === "BOARDING")
      return (
        <button
          type="button"
          ...
          className="driver-primary-action"
        >
          <QrCode aria-hidden /> Scan boarding pass
        </button>
      );
    if (manifest.currentStop && alightHere.length > 0)
      return (
        <button
          type="button"
          ...
          className="driver-primary-action"
        >
          <DoorOpen aria-hidden /> Scan alighting pass
        </button>
      );
    if (manifest.trip.status === "DEPARTED" && !manifest.currentStop)
      return (
        <button
          type="button"
          onClick={() => setPendingProgress("ARRIVE_NEXT_STOP")}
          className="driver-primary-action"
        >
          <MapPin aria-hidden /> Arrive next stop
        </button>
      );
    if (manifest.currentStop)
      return (
        <button
          type="button"
          onClick={() => setPendingProgress("DEPART_CURRENT_STOP")}
          className="driver-primary-action"
        >
          <Route aria-hidden /> Depart {manifest.currentStop.name}
        </button>
      );
    return null;
  }
    ...
    <div className="driver-shell">
      ...
      <main id="main-content" className="driver-content">
        ...
        <MotionSurface motionKey={view}>
          {loading && (
            <div className="driver-empty">Loading assigned Trips…</div>
          )}
      ...
      {scannerMode && activeTripId && (
        <QRScannerModal
          tripId={activeTripId}
          routeName={manifest?.trip.routeName}
          currentStopName={manifest?.currentStop?.name ?? undefined}
          mode={scannerMode}
          onClose={() => setScannerMode(null)}
          onSuccess={() => void refreshManifest(activeTripId)}
        />
      )}
      ...
      <ConfirmModal
        isOpen={pendingProgress !== null}
        onClose={() => setPendingProgress(null)}
        onConfirm={() => {
          if (pendingProgress) void progress(pendingProgress);
          setPendingProgress(null);
        }}
        title={
          pendingProgress === "DEPART_CURRENT_STOP"
            ? "Depart current stop?"
            : "Arrive at next stop?"
        }
        message={
          pendingProgress === "DEPART_CURRENT_STOP"
            ? "Boarding at this stop will close when the Trip departs."
            : "Confirm that the shuttle has arrived at the next stop."
        }
        confirmText="Confirm progress"
        cancelText="Go back"
      />
    </div>
  );
