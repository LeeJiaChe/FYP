...
test("student creates a reserved journey and opens its Reserved Pass", async ({ page }) => {
  ...
  await openJourney(page, "Wangsa Maju Section 2 → TAR UMT");
  ...
  const bookingArticle = page.getByRole("article").filter({ hasText: "Wangsa Maju Section 2 → TAR UMT" });
  await expect(
    bookingArticle.getByRole("heading", { name: "Wangsa Maju Section 2 → TAR UMT" }),
  ).toBeVisible();
